...
const TABS = ['OPERATIONS', 'GUEST EXPERIENCE', 'F&B', 'FINANCE'] as const;
    ...
    <section className="bg-white py-6">
      <div className="container grid lg:grid-cols-2 gap-6">
        <div className="bg-black text-white rounded-2xl p-10 md:p-16 lg:p-20 flex flex-col justify-center relative overflow-hidden">
          {/* Decorative element */}
          <div className="absolute top-1/2 -left-10 w-9 h-9 text-supporting-grey -translate-y-1/2" aria-hidden="true">
        ...
        <div className="bg-pink-lightest rounded-2xl overflow-hidden relative aspect-square lg:aspect-auto">
          ...
          <div className="absolute bottom-8 left-8 right-8 flex flex-wrap gap-2" role="tablist" aria-label="Product categories">
            {TABS.map((tab) => (
              <button
                key={tab}
      ...
      <AnimatePresence>
        {isVideoOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" role="dialog" aria-modal="true" aria-label="Video tutorial">
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
            ...
            >
              <button
                onClick={() => setIsVideoOpen(false)}
                className="absolute top-4 right-4 z-10 p-2 bg-white/10 hover:bg-white/20 text-white rounded-full transition-colors min-w-[44px] min-h-[44px] flex items-center justify-center"
